...
def runEnvironmentMonitoringAgent(cloudevent):
    """
    Cloud Function entry point to handle Pub/Sub messages for environmental monitoring.
    """
    ...
    hazard_types = payload.get("hazard_types", "all")
    
    # Generate the prompt for environmental information
    example_prompt = (
        f"My current location is {lat}, {lon}. Provide environmental hazard information for "
        f"{areas} from the {timeframe}. Focus on {hazard_types} types of hazards. "
        f"Include weather alerts, air quality, and environmental hazards."
    )
    
    logger.info("sending prompt to gemini: %s", example_prompt)
    
    # Get the environment digest based on the generated prompt
    digest = get_environment_digest(example_prompt)
    logging.info("Environment digest generated:\n%s", digest)
